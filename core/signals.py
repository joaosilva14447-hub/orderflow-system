...
class SignalEngine:
    ...
    def detect(self, bars: list[Bar]) -> list[Signal]:
        ...
        for i in range(self.vp_window + lb, len(bars)):
            ...
            trend_bear_ok = sma is None or b.close > sma   # above SMA → in uptrend → reversal short valid
            trend_bull_ok = sma is None or b.close < sma   # below SMA → in downtrend → reversal long valid

            if price_hh and cvd_lh and trend_bear_ok:
            ...
            elif price_ll and cvd_hl and trend_bull_ok:
